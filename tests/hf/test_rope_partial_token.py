...
but built with a partial rotary width (``d_rot < d_head``). It compiles
directly to stock Phi-3 and checks the HF runtime:

* carries ``d_rot`` (and ``rope_base``) through the direct compiler,
* still predicts the previous token (the partial rotation survives export +
  compilation — the NoPE tail is a position-independent constant the softmax ignores),
...
* a **content head** (``attend_argmax_dot``) whose content rides the NoPE tail
  survives direct compilation and still selects by content (predicts the
...
def test_torchwright_config_validates_d_rot():
    ...
    with pytest.raises(ValueError, match="d_rot"):
        TorchwrightCustomConfig(d=256, d_head=16, d_rot=3)
    with pytest.raises(ValueError, match="d_rot"):
        TorchwrightCustomConfig(d=256, d_head=16, d_rot=18)
    # Valid partial width is accepted and stored.
    assert TorchwrightCustomConfig(d=256, d_head=16, d_rot=8).d_rot == 8
...
def _build_content_model(tmpdir):
    ...
    return compile_to_hf(out, emb, d=D, d_head=D_HEAD, max_seq_len=64)
