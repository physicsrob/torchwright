...
*previous* token.  Exports to ONNX, converts to the native
``TorchwrightForCausalLM``, and checks the HF runtime:

* carries the RoPE config (``rope_base`` + per-head enable) through the converter,
* predicts the previous token (the rotary selection survives export + convert),
