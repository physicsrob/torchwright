...
def test_negative_saturation_exact_zero_from_minus_18():
    """ORT-CPU sigmoid is exactly 0.0 for every z <= -18.

    Torch keeps denormals down to ~-103, so this is strictly safer
    for the spec's leak claims: sigma(-128) = 0 (dead select branch)
    is included.
    """
...
def test_swish_fixed_points_and_onehot_indicator():
    """The claims the spec leans on hold on this kernel.

    Swish(0) = 0, Swish(128) = 128 (saturated winning gate), Swish(64)
    = 64 (onehot winner indicator = exactly 0.5 after /scale), and the
    onehot leak at -0.5 is exactly zero (sigma(-64) = 0 here, unlike
    torch).
    """
...
def test_bias_lane_constants_exact_unit_lane():
    ...
    sigma(bias_lane_gate) is exactly 1.0 (input 32 sits comfortably
    past this kernel's late exact-1.0 threshold of 18), and the full
    lane expression — the GatedMLPSubLayer's ``g * sigmoid(g) * u`` —
