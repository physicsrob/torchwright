...
#: power of two, so folded k/scale weights and x-scale/÷-scale round trips
...
def test_scale_is_the_module_constant():
    """The value every claim below is derived at IS the shipped module constant.

    That's ``ops/const.py`` — the one the compiler's swish bypass pair
    and the swiglu ops fold into weights.
    """
...
def test_swish_dip_is_the_module_constant():
    """The dip magnitude (ops/const.py) IS the doc's 0.2785.

    This is the value the swiglu ops size their assert slacks and
    semantic-bound widenings with, and both match the actual minimum
    of z·sigmoid(z).
    """
...
def test_min_d_hidden_is_the_module_constant():
    """The declared minimum MLP hidden width IS the shipped module constant.

    That's ``ops/const.py``. The flagship compiles at d_hidden = 16384
...
def test_swiglu_chunk_caps_derive_from_min_d_hidden():
    """The three swiglu lookup-family chunk caps are wired to min_d_hidden.

    Not hardcoded: piecewise_linear's d_max default is the constant
    itself; floor_int and table_lookup_2d chunk at min_d_hidden // 2
    (two hinge lanes per boundary). A hardcoded 1024 or 512 would pass
    a value check but silently decouple from the constant, so these
    pin the derivation in source.
    """
    import inspect
...
def test_sharpened_hinge_uniform_bound():
    ...
    It peaks at z = -1.278/scale (Swish's argmin).
    """
...
def test_naive_transliteration_fails():
    """compare: the unsharpened ramp Swish(z) - Swish(z-1) misses its levels.

    By 0.27 at the contract points and ~0.10 two ramp-widths past
...
def test_fp32_sigmoid_saturation_threshold():
    """Preamble: fp32 sigmoid computes exactly 1.0 once its input exceeds ~17.

    e^-17 is below fp32's resolution next to 1; 16 is not enough.
    """
...
def test_compare_contract_points_bit_exact_fp32():
    """compare: at scale=128 in fp32, the contract-point outputs are bit-exact.

    +1/-1, because both hinges are saturated or on-bend.
    """
...
def test_multiply_identity_exact():
    ...
    This is the sigma(a) + sigma(-a) = 1 cancellation.
    """
...
def test_abs_integer_grid_bit_exact_fp32():
    ...
    Because of tanh/sigmoid saturation.
    """
...
def test_min_hinge_form():
    ...
    Ties are exact, and the error is symmetric in the arguments
    despite the asymmetric construction.
    """
...
def test_gated_select_off_branch_exact_zero_fp32():
    """broadcast_select/select: at mask=-1, scale=128 fp32, the losing branch is zero.

    sigmoid(-128) computes as 0.0 on the CPU kernel, and
    e^-128 ~ 2.6e-56 sits below fp32's subnormal floor (~1.4e-45), so
    even a kernel that materialized the exponential would underflow to
    exactly 0.
    """
...
def test_gated_select_winning_branch_bit_exact():
    """broadcast_select: at mask=+1 the winning branch is bit-exact at scale=128.

    The value rides through x scale then / scale, and both factors are
    powers of two, so neither product rounds. (At scale=100 this
    carried up to 1 fp32 ulp relative rounding, the recorded regression
    versus the approximate=False path; the power-of-two scale erases
    it.)
    """
...
def test_gated_select_mask_deviation_passthrough():
    """broadcast_select: a saturated gate IS the mask.

    sigma == 1.0 for scale*m >= 17, i.e. m >= 0.14 at scale=128, so a
    mask off +1 by delta mis-scales the winner by exactly delta *
    value — actual value, not the range maximum M. With the
    power-of-two scale the saturated gate equals the mask bit for bit.
    """
...
def test_relu_cancellation_absolute_error_at_M():
    """broadcast_select what-dies note: approximate=True recovers via (M+t)-M.

    In fp32, absolute error up to half an ulp of M — 3e-5 at M=1000 —
    error at the offset's magnitude even for tiny values, versus the
    gated form's relative-to-the-value ulp.
    """
...
def test_table_lookup_2d_telescoping():
    """table_lookup_2d: the swish form recovers integer-grid entries and edge clamps.

    Per-axis two-stage staircases, the column axis consuming its steps
    through gated lanes (gate = hinge(1 - step), up = adjacent-column
    difference), exactly, and yields genuine bilinear interpolation
    inside boundary bands (today's op disclaims bilinear).
    """
    s, W = 100.0, 2.0
    dt = torch.float64
...
def test_onehot_lookup_counting_margin():
    ...
    At argument +0.5 (winner) or <= -0.5 (rest). Sharpened: hinge(0.5)
    is exactly 0.5 in fp32; hinge(-0.5) leaks ~1e-28 per row (e^-64 is
    representable, unlike sigma(-128) = 0); a slightly-off one-hot
    shifts the indicator by exactly its count deviation (saturated
    gate).
    """
...
def test_scalar_to_embedding_staircase():
    """scalar_to_embedding: the 9-unit-step staircase ports hinge-for-hinge.

    This is the piecewise_linear special case. Integer digits put
    every hinge argument on an exact saturated integer, so component
    error is out_proj rounding only (~few ulps at norm-40 embeddings);
    mid-ramp blends the adjacent embeddings; the pair-spacing audit
    reduces to scale > 34.
    """
...
def test_hinge_fillet_width():
    """Entries' fillet radius: beyond |z| ~ 17/scale, hinge deviation is tiny.

    Below 1e-8 from ReLU — the '17/(scale*sharpness)' error zones.
    """
...
def test_bias_lane_constants_exact_unit_lane():
    """The no-bias constant lane (docs/no_bias_plan.md): constants are powers of two.

    Both shipped constants (ops/const.py) are powers of two, the gate
    value saturates sigma bit-exactly (>= 17 on this kernel; the
    CPU-ORT floor of 18 also clears), and the full lane expression —
    the GatedMLPSubLayer's ``g * sigmoid(g) * u`` — computes exactly
    1.0 in fp32, so a constant routed through the lane's
    down-projection row lands verbatim.
    """
