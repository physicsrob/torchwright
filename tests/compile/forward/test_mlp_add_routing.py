...
def test_default_and_legacy_policies_retain_attention_add_ops(policy):
    """Keep Adds on attention under both the shipping default and LEGACY_POLICY.

    The 2026-07-14 fallback decision: a static MLP default costs one layer
    per Add wedged between MLP-sublayer ops.
    """
...
def test_mlp_routed_adds_match_compute():
    """Match the recursive oracle through compile_headless with MLP-routed Adds.

    Per-machine ReLU/Swish coverage of the writers lives in
    test_weight_writer.py; the machine here is whatever the default
    compile selects.
    """
    out, *_ = _mixed_add_graph()
...
def test_corrupted_observation_trips_the_derivation_tripwire():
    """Raise the named invariant before returning an assignment on a corrupted trace.

    A trace whose observed Add placement disagrees with the
    assignment-level derivation must raise — for the fresh/reused form and
    for the occurrence, including add(x, x).
    """
    x = create_input("x", 4, value_range=(-1.0, 1.0))
