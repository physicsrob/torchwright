...
def test_rotary_offset_token_identical_to_trig(delta):
    ...
    This holds at every in-bounds position: the sign-convention lock
    across all Δ.
    """
...
def test_rotary_offset_compiled_matches_oracle_all_deltas(delta):
    """probe_compiled: the compiled rotary head matches its oracle.

    Checked at representative backward, wider-backward, and forward Δ.
    """
    payload = create_input("payload", 1)
    vals = _payload(N_POS)
    rotary = rotary_offset_head(payload, delta_pos=delta, d_qk=D_HEAD)
