...
# (d, d_hidden): the canonical family geometry first (what the layer table
# and the publish path compile at — examples/_calculator_common.py), then
# width-study rows on either side of it (narrow, starved geometries below;
# the doom-flagship width above).  d_head is NOT swept: it is baked into
# the graph at build time (every Attn's d_qk must equal the compile
# d_head), so changing it means rebuilding with a new module D_HEAD, not
# passing a different compile knob.
DEFAULT_CONFIGS = [
    (8192, 16384),  # canonical (also the doom-flagship widths)
    (1024, 1024),  # width study: starved
    (2048, 4096),  # width study: narrow
    (4096, 8192),  # width study: half-canonical
]
DEFAULT_NS = [3, 6]
...
def main() -> None:
    ...
    ap.add_argument(
        "--configs",
        default=None,
        help="comma-separated d:d_hidden pairs (e.g. 4096:8192); default sweeps "
        "DEFAULT_CONFIGS",
    )
    args = ap.parse_args()
    ...
    configs = (
        [tuple(int(x) for x in pair.split(":")) for pair in args.configs.split(",")]
        if args.configs
