...
def _freeze_ints(value, field_name: str) -> tuple[int, ...]:
    ...
    if any(
        not isinstance(item, Integral) or isinstance(item, bool) for item in frozen
    ):
...
def _freeze_node_indices(value, field_name: str, *, sort: bool) -> "NodeIndices":
    ...
    for item in items:
        if not isinstance(item, (tuple, list)) or len(item) != 2:
            raise TypeError(
                f"{field_name} entries must be (node_id, indices) pairs"
            )
        ...
        seen.add(node_id)
        frozen.append(
            (node_id, _freeze_ints(indices, f"{field_name}[{node_id}]"))
        )
    if sort:
...
class PlannedLayer:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "biased_linear_ids", frozenset(self.biased_linear_ids)
        )
        object.__setattr__(
            self,
            "residual_snapshot",
...
class ReplayPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if not layers or any(
            not isinstance(layer, PlannedLayer) for layer in layers
        ):
